

# Class: DtAllPhyloTaxonStats 



URI: [img_core_v400:DtAllPhyloTaxonStats](https://w3id.org/jgi/img_core_v400/DtAllPhyloTaxonStats)





```mermaid
 classDiagram
    class DtAllPhyloTaxonStats
    click DtAllPhyloTaxonStats href "../DtAllPhyloTaxonStats/"
      DtAllPhyloTaxonStats : data_type
        
      DtAllPhyloTaxonStats : est_copy_30
        
      DtAllPhyloTaxonStats : est_copy_60
        
      DtAllPhyloTaxonStats : est_copy_90
        
      DtAllPhyloTaxonStats : gene_count_30
        
      DtAllPhyloTaxonStats : gene_count_60
        
      DtAllPhyloTaxonStats : gene_count_90
        
      DtAllPhyloTaxonStats : homolog_taxon
        
      DtAllPhyloTaxonStats : taxon_oid
        
      
```




<!-- no inheritance hierarchy -->


## Slots

| Name | Cardinality and Range | Description | Inheritance |
| ---  | --- | --- | --- |
| [taxon_oid](taxon_oid.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [data_type](data_type.md) | 0..1 <br/> [String](String.md) |  | direct |
| [homolog_taxon](homolog_taxon.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [gene_count_30](gene_count_30.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [gene_count_60](gene_count_60.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [gene_count_90](gene_count_90.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [est_copy_30](est_copy_30.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [est_copy_60](est_copy_60.md) | 0..1 <br/> [Float](Float.md) |  | direct |
| [est_copy_90](est_copy_90.md) | 0..1 <br/> [Float](Float.md) |  | direct |










## Identifier and Mapping Information






### Schema Source


* from schema: https://w3id.org/jgi/img_core_v400




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | img_core_v400:DtAllPhyloTaxonStats |
| native | img_core_v400:DtAllPhyloTaxonStats |






## LinkML Source

<!-- TODO: investigate https://stackoverflow.com/questions/37606292/how-to-create-tabbed-code-blocks-in-mkdocs-or-sphinx -->

### Direct

<details>
```yaml
name: dt_all_phylo_taxon_stats
from_schema: https://w3id.org/jgi/img_core_v400
attributes:
  taxon_oid:
    name: taxon_oid
    from_schema: https://w3id.org/jgi/img_core_v400
    domain_of:
    - asv5_taxons
    - bcg_taxons
    - dt_all_phylo_taxon_stats
    - dt_cog_stats
    - dt_kog_stats
    - dt_phylo_taxon_stats
    - dt_phylodist_new_taxons
    - dt_phylum_dist_genes
    - dt_phylum_dist_stats
    - dt_scog_genes
    - dt_taxon_kmodule_mcr
    - img_iuig_tarballs_11202023
    - img_umag_bin_tarballs_02062024
    - imgnr_taxons_062123
    - iso_plasmids
    - kp_actino
    - kp_taxons
    - new_taxons
    - new_taxons_batch
    - old_taxons
    - tarball_refresh_taxons
    - taxon
    - taxon_assembly_stats
    - taxon_cathfunfam_count
    - taxon_cog_count
    - taxon_crispr_details
    - taxon_crispr_summary
    - taxon_dw
    - taxon_ec_count
    - taxon_ext_links
    - taxon_gene_prefix
    - taxon_gtdbtk_lineage
    - taxon_ko_count
    - taxon_pfam_count
    - taxon_prod_vw
    - taxon_replacements
    - taxon_replacements_view
    - taxon_scaf_prefix
    - taxon_smart_count
    - taxon_smc_stats
    - taxon_stats
    - taxon_stats_merfs
    - taxon_stats_prod_vw
    - taxon_supfam_count
    - taxon_taxon_scaffold_info
    - taxon_tigr_count
    - vw_gold_taxon
    - vw_taxon_sc
    range: float
    required: false
  data_type:
    name: data_type
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    - taxon_cathfunfam_count
    - taxon_cog_count
    - taxon_ec_count
    - taxon_gene_prefix
    - taxon_ko_count
    - taxon_pfam_count
    - taxon_scaf_prefix
    - taxon_smart_count
    - taxon_supfam_count
    - taxon_tigr_count
    range: string
    required: false
  homolog_taxon:
    name: homolog_taxon
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    - dt_phylum_dist_genes
    range: float
    required: false
  gene_count_30:
    name: gene_count_30
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  gene_count_60:
    name: gene_count_60
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  gene_count_90:
    name: gene_count_90
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  est_copy_30:
    name: est_copy_30
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  est_copy_60:
    name: est_copy_60
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  est_copy_90:
    name: est_copy_90
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false

```
</details>

### Induced

<details>
```yaml
name: dt_all_phylo_taxon_stats
from_schema: https://w3id.org/jgi/img_core_v400
attributes:
  taxon_oid:
    name: taxon_oid
    from_schema: https://w3id.org/jgi/img_core_v400
    alias: taxon_oid
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - asv5_taxons
    - bcg_taxons
    - dt_all_phylo_taxon_stats
    - dt_cog_stats
    - dt_kog_stats
    - dt_phylo_taxon_stats
    - dt_phylodist_new_taxons
    - dt_phylum_dist_genes
    - dt_phylum_dist_stats
    - dt_scog_genes
    - dt_taxon_kmodule_mcr
    - img_iuig_tarballs_11202023
    - img_umag_bin_tarballs_02062024
    - imgnr_taxons_062123
    - iso_plasmids
    - kp_actino
    - kp_taxons
    - new_taxons
    - new_taxons_batch
    - old_taxons
    - tarball_refresh_taxons
    - taxon
    - taxon_assembly_stats
    - taxon_cathfunfam_count
    - taxon_cog_count
    - taxon_crispr_details
    - taxon_crispr_summary
    - taxon_dw
    - taxon_ec_count
    - taxon_ext_links
    - taxon_gene_prefix
    - taxon_gtdbtk_lineage
    - taxon_ko_count
    - taxon_pfam_count
    - taxon_prod_vw
    - taxon_replacements
    - taxon_replacements_view
    - taxon_scaf_prefix
    - taxon_smart_count
    - taxon_smc_stats
    - taxon_stats
    - taxon_stats_merfs
    - taxon_stats_prod_vw
    - taxon_supfam_count
    - taxon_taxon_scaffold_info
    - taxon_tigr_count
    - vw_gold_taxon
    - vw_taxon_sc
    range: float
    required: false
  data_type:
    name: data_type
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: data_type
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    - taxon_cathfunfam_count
    - taxon_cog_count
    - taxon_ec_count
    - taxon_gene_prefix
    - taxon_ko_count
    - taxon_pfam_count
    - taxon_scaf_prefix
    - taxon_smart_count
    - taxon_supfam_count
    - taxon_tigr_count
    range: string
    required: false
  homolog_taxon:
    name: homolog_taxon
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: homolog_taxon
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    - dt_phylum_dist_genes
    range: float
    required: false
  gene_count_30:
    name: gene_count_30
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: gene_count_30
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  gene_count_60:
    name: gene_count_60
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: gene_count_60
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  gene_count_90:
    name: gene_count_90
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: gene_count_90
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  est_copy_30:
    name: est_copy_30
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: est_copy_30
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  est_copy_60:
    name: est_copy_60
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: est_copy_60
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false
  est_copy_90:
    name: est_copy_90
    from_schema: https://w3id.org/jgi/img_core_v400
    rank: 1000
    alias: est_copy_90
    owner: dt_all_phylo_taxon_stats
    domain_of:
    - dt_all_phylo_taxon_stats
    - dt_phylo_taxon_stats
    range: float
    required: false

```
</details>