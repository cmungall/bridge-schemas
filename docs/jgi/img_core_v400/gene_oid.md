

# Slot: gene_oid 



URI: [img_core_v400:gene_oid](https://w3id.org/jgi/img_core_v400/gene_oid)
Alias: gene_oid

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [GeneTigrfams](GeneTigrfams.md) |  |  no  |
| [GeneAliases](GeneAliases.md) |  |  no  |
| [GeneParalogs](GeneParalogs.md) |  |  no  |
| [GeneFragCoords](GeneFragCoords.md) |  |  no  |
| [GeneSmart](GeneSmart.md) |  |  no  |
| [GenePdbXrefs](GenePdbXrefs.md) |  |  no  |
| [TmpGenes](TmpGenes.md) |  |  no  |
| [GeneCogGroups](GeneCogGroups.md) |  |  no  |
| [GeneImgInterproHits](GeneImgInterproHits.md) |  |  no  |
| [MerfsGeneMapping](MerfsGeneMapping.md) |  |  no  |
| [MvGeneTmhmmStats](MvGeneTmhmmStats.md) |  |  no  |
| [GeneExceptions](GeneExceptions.md) |  |  no  |
| [GenePfamFamilies](GenePfamFamilies.md) |  |  no  |
| [GeneXrefFamilies](GeneXrefFamilies.md) |  |  no  |
| [DeleteGenes](DeleteGenes.md) |  |  no  |
| [GeneKoEnzymes](GeneKoEnzymes.md) |  |  no  |
| [GeneSigPeptides](GeneSigPeptides.md) |  |  no  |
| [GeneExtLinks](GeneExtLinks.md) |  |  no  |
| [DtScogGenes](DtScogGenes.md) |  |  no  |
| [GeneGoTerms](GeneGoTerms.md) |  |  no  |
| [GeneKoEnzymesNew](GeneKoEnzymesNew.md) |  |  no  |
| [KpPseudo](KpPseudo.md) |  |  no  |
| [GeneCathfam](GeneCathfam.md) |  |  no  |
| [GeneSuperfam](GeneSuperfam.md) |  |  no  |
| [ActinoAbcGenes](ActinoAbcGenes.md) |  |  no  |
| [DtImgGeneProtPepSample](DtImgGeneProtPepSample.md) |  |  no  |
| [GeneEssentialGenes](GeneEssentialGenes.md) |  |  no  |
| [GeneKoTerms](GeneKoTerms.md) |  |  no  |
| [DtPhylumDistGenes](DtPhylumDistGenes.md) |  |  no  |
| [Gene](Gene.md) |  |  no  |
| [GeneFeatureTags](GeneFeatureTags.md) |  |  no  |
| [GeneNotes](GeneNotes.md) |  |  no  |
| [DtHtHits](DtHtHits.md) |  |  no  |
| [GeneBiocycRxns](GeneBiocycRxns.md) |  |  no  |
| [GeneKogGroups](GeneKogGroups.md) |  |  no  |






## Properties

* Range: [String](String.md)




## Identifier and Mapping Information







## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | img_core_v400:gene_oid |
| native | img_core_v400:gene_oid |




## LinkML Source

<details>
```yaml
name: gene_oid
alias: gene_oid
domain_of:
- actino_abc_genes
- delete_genes
- dt_ht_hits
- dt_img_gene_prot_pep_sample
- dt_phylum_dist_genes
- dt_scog_genes
- gene
- gene_aliases
- gene_biocyc_rxns
- gene_cathfam
- gene_cog_groups
- gene_essential_genes
- gene_exceptions
- gene_ext_links
- gene_feature_tags
- gene_frag_coords
- gene_go_terms
- gene_img_interpro_hits
- gene_ko_enzymes
- gene_ko_enzymes_new
- gene_ko_terms
- gene_kog_groups
- gene_notes
- gene_paralogs
- gene_pdb_xrefs
- gene_pfam_families
- gene_sig_peptides
- gene_smart
- gene_superfam
- gene_tigrfams
- gene_xref_families
- kp_pseudo
- merfs_gene_mapping
- mv_gene_tmhmm_stats
- tmp_genes
range: string

```
</details>