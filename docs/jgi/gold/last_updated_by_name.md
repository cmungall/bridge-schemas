

# Slot: last_updated_by_name 



URI: [gold:last_updated_by_name](https://w3id.org/jgi/gold/last_updated_by_name)
Alias: last_updated_by_name

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [DwSamples](DwSamples.md) |  |  no  |
| [DwSequencingProduct](DwSequencingProduct.md) |  |  no  |
| [DwSequencingProject](DwSequencingProject.md) |  |  no  |






## Properties

* Range: [String](String.md)




## Identifier and Mapping Information







## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | gold:last_updated_by_name |
| native | gold:last_updated_by_name |




## LinkML Source

<details>
```yaml
name: last_updated_by_name
alias: last_updated_by_name
domain_of:
- dw_samples
- dw_sequencing_product
- dw_sequencing_project
range: string

```
</details>